...
function ENT:Initialize()
	...
	self.Time = CurTime() + LPrintTwo.Config.Gold.TypePerCycle // Immediently start a cycle.
...
function ENT:Think()
	...
	if(self.Time <= CurTime()) then
		...
		// Add money + heat
		self:SetMoney(self:GetMoney() + LPrintTwo.Config.Gold.MoneyPerCycle)

		self.Time = CurTime() + LPrintTwo.Config.Gold.TypePerCycle // Reset the time.
