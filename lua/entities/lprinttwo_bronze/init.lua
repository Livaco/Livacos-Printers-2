...
function ENT:Initialize()
	...
	self:SetColor(Color(210, 50, 50, 255))
	...
	self.Time = CurTime() + LPrintTwo.Config.Bronze.TypePerCycle // Immediently start a cycle.
...
function ENT:Think()
	...
	if(self.Time <= CurTime()) then
		...
		// Add money + heat
		self:SetMoney(self:GetMoney() + LPrintTwo.Config.Bronze.MoneyPerCycle)

		self.Time = CurTime() + LPrintTwo.Config.Bronze.TypePerCycle // Reset the time.
