...
function ENT:Initialize()
	...
	self:SetColor(Color(20, 20, 255, 255))
	...
	self.Time = CurTime() + LPrintTwo.Config.Diamond.TypePerCycle // Immediently start a cycle.
...
function ENT:Think()
	...
	if(self.Time <= CurTime()) then
		...
		// Add money + heat
		self:SetMoney(self:GetMoney() + LPrintTwo.Config.Diamond.MoneyPerCycle)

		self.Time = CurTime() + LPrintTwo.Config.Diamond.TypePerCycle // Reset the time.
